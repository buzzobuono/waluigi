...
DATASET_ID = "test/unit/dataset_test"
...
def test_update_description():
    catalog.create_dataset(DATASET_ID, format="csv", description="original")
    catalog._patch(f"/datasets/{DATASET_ID}", json={"description": "updated"})
    assert catalog.get_dataset(DATASET_ID)["description"] == "updated"


def test_update_status():
    catalog.create_dataset(DATASET_ID, format="csv")
    catalog._patch(f"/datasets/{DATASET_ID}", json={"status": "in_review"})
...
def test_delete_nonexistent_dataset():
    with pytest.raises(CatalogError):
        catalog._delete("/datasets/does/not/exist")


# ── Filter ────────────────────────────────────────────────────────────────────

def test_filter_by_status():
    catalog.create_dataset(DATASET_ID, format="json", description="filter test")
...
def test_filter_by_description():
    catalog.create_dataset(DATASET_ID, format="json", description="unique-filter-marker")
    results = catalog.list_datasets(description="unique-filter-marker")
    assert any(d["id"] == DATASET_ID for d in results)


def test_filter_excludes_other_status():
    catalog.create_dataset(DATASET_ID, format="json")
...
def test_status_lifecycle_draft_to_in_review():
    catalog.create_dataset(DATASET_ID, format="csv")
    catalog._patch(f"/datasets/{DATASET_ID}", json={"status": "in_review"})
    assert catalog.get_dataset(DATASET_ID)["status"] == DatasetStatus.IN_REVIEW


def test_status_lifecycle_in_review_to_approved():
    catalog.create_dataset(DATASET_ID, format="csv")
    catalog._patch(f"/datasets/{DATASET_ID}", json={"status": "in_review"})
    catalog._patch(f"/datasets/{DATASET_ID}", json={"status": "approved"})
    assert catalog.get_dataset(DATASET_ID)["status"] == DatasetStatus.APPROVED


def test_status_lifecycle_to_deprecated():
    catalog.create_dataset(DATASET_ID, format="csv")
    catalog._patch(f"/datasets/{DATASET_ID}", json={"status": "deprecated"})
...
def test_approve_changes_status():
    ...
    result = catalog._post(f"/datasets/{DATASET_ID}/_approve",
                           json={"approved_by": "tester", "notes": "looks good"})
    assert result["status"] == "approved"
    assert result["approved_by"] == "tester"


def test_approve_after_commit(sample_rows_for_approval):
