...
class CatalogDB:
        ...
        with self.conn:
            self.conn.executescript("""
                ...
                CREATE INDEX IF NOT EXISTS idx_versions_status
                    ON versions(dataset_id, status);
                CREATE INDEX IF NOT EXISTS idx_versions_committed
                    ON versions(dataset_id, committed_at);
                CREATE INDEX IF NOT EXISTS idx_lineage_output
                    ON lineage(output_dataset, output_version);
                CREATE INDEX IF NOT EXISTS idx_lineage_input
                    ON lineage(input_dataset, input_version);
                CREATE INDEX IF NOT EXISTS idx_datasets_prefix
                    ON datasets(id);
            """)
    ...
    def list_by_status(self, status: str) -> list[dict]:
        """List all datasets with a given status."""
        cur = self.conn.execute("""
            SELECT d.*,
                   v.version AS latest_version,
                   v.format, v.rows, v.committed_at
            FROM datasets d
            LEFT JOIN versions v
                ON v.dataset_id = d.id
                AND v.version = (
